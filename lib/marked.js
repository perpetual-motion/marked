...
(function () {
    ...
    block.token = function (src, tokens, top) {
        ...
        while (src) {
            ...
            for (each in block.plugins) {
                if (cap = block.plugins[each].search.exec(src)) {
                    src = src.substring(cap[0].length);
                    tokens.push({
                        type: block.plugins[each].replace,
                        capture: cap
                    });
                    continue;
                }
    ...
    inline.lexer = function (src) {
        ...
        while (src) {
            ...
            // inline plugins merely replace their contents with something else (could be html or more markdown)
            for (each in inline.plugins) {
                src = src.replace(inline.plugins[each].search, inline.plugins[each].replace);
                continue;
            }
    ...
    function tok() {
        // poach for plugins first.
        if (typeof token.type === 'function') {
            return token.type.apply(null, token.capture);
        }
    ...
    marked.pygments_highlight = function (source, language, callback) {
        var text = '';
    ...
    marked.async = function (src, func) {
        ...
        // if not set, it uses the provided pygments highlighter.
        if (!highlight || highlight.length < 3) {
            highlight = marked.pygments_highlight;
        }
    ...
    function setOptions(opt) {
        ...
        block.plugins = options.blocks || {};
        inline.plugins = options.inlines || {};
    }
